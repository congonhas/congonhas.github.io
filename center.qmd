...
One consequence deserves stating, because it bears on how findings travel. An impact is not a property of a program. It is a property of a program delivered by particular people with particular skills, and those skills are unevenly distributed and rarely funded. We can say what this program produces when it is delivered well, because we scored every session against a fidelity scale. Very few estimates can say that, and a district that adopts a program on the strength of published effects, without the capacity to deliver it, may get very little and have no way to tell whether the idea failed or the delivery did.

There is a further distinction worth stating plainly. A research and development system has to assign value to informative failure. That does not make failure desirable, and an intervention that fails while teaching nothing is simply a failure. But an intervention that fails while resolving an important uncertainty improves the next decision, and a program can generate impressive activity while leaving decision makers knowing no more than they knew before about what should happen next. Judged by output, the second looks better. Judged by what it contributes to the next choice, the first often is.
