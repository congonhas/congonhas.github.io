...
There is one implication we want to state directly, because it concerns how evidence reaches the people who use it. The What Works Clearinghouse and others like it exist for a good reason. A superintendent cannot read the literature, and a list of programs rated by what is known about them is a genuine service. But a rating is a verdict on a program, and an impact is not a property of a program. It is a property of a program delivered by particular people with particular skills. Those skills are unevenly distributed, they are rarely funded, and nothing in a rating tells a district whether it has them.

The consequence is practical. A district that adopts a well-rated program without what Alief spent three years building may get very little, and will have no way to tell whether the idea failed or the delivery did. We can say what this program produces when it is delivered well, because we scored every session against a fidelity scale. Very few estimates can say that. The honest form of a rating would carry the condition along with the verdict: this is what the program produced, delivered this way, by staff prepared in this way.
